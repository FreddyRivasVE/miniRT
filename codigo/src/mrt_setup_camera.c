...
/*   Updated: 2025/08/14 10:39:23 by frivas           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_camera_view mrt_compute_camera_view(t_camera *cam, t_window win)
{
    ...
    float aspect_ratio = (float)win.width / (float)win.height;
    float theta = cam->fov * PI / 180.0f; // FOV en radianes
    ...
    t_vec4 world_up = vec4_create(0.0f, 1.0f, 0.0f, 0.0f);
    t_vec4 forward = vec4_normalize(cam->direction); // mirando hacia adelante
    t_vec4 right = vec4_normalize(vec4_cross(forward, world_up));
    t_vec4 up_cam = vec4_cross(right, forward);
