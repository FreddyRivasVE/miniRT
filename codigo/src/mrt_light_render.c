...
/*   Updated: 2025-08-23 12:46:37 by brivera          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
bool	mrt_intersect_scene(t_data *elements, t_ray *ray, t_hittable **shadow_hit)
{
	...
	temp_hit = ft_calloc(1, sizeof(t_hittable));
	while (current)
	{
...
t_vec4  mrt_light_color(t_data *elements, t_hittable *hit, t_ray *ray)
{
	...
	light_dist = vect4_lenght(vec4_sub(elements->light->position, hit->point));
	light_dir = vec4_normalize(vec4_sub(elements->light->position, hit->point));
	shadow_ray.direction = light_dir;
	shadow_ray.origin = vec4_add(hit->point, vec4_scale(hit->normal, EPSILON));
	ambient = vec4_scale(elements->ambient->color, elements->ambient->ratio);
	//ver si puede poner el init_scene
	diff = (t_vec4){0, 0, 0, 0};
	spec = (t_vec4){0, 0, 0, 0};
	hit_color = (t_vec4){0, 0, 0, 0};
	if (!mrt_intersect_scene(elements, &shadow_ray, &shadow_hit) || shadow_hit->t > light_dist)
	{
	...
	hit_color = vec4_mul(hit_color, hit->material->base_color);
	return (hit_color);
}
